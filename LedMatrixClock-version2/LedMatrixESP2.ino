...
CRGB leds[NUM_LEDS];                                  // THE TOTAL NUMBER OF LEDS USED IN THE MATRIX (should be XRES * YRES)
std::map<int, CRGB> timeMatrix;                       // This creates a Map (sort of array) containing, the led numbers that are part of the digits
std::map<int, CRGB> bgMatrix;                         // This creates a Map of the bg LEDs (= all leds - digits)
...
/*
   ...
   Followed by initializing the matrix and leds
...
void loop() {
  ...
  EVERY_N_SECONDS(60) {                               // Every minute we check if the clock has to go to nightmode, a mode with low light and no animations.
    nightModeOnOrOff();
  }
  ...
  if (randomAnimation == "No Random animation") {     //  If we want a random animation every ... minutes.. let's check if its time.
    activity = whichFX;
  ...
  */
  ...
  if ( nightMode ) {                                                  // If the clock is in Nightmode, we don't need much logic. only display the digits in faint red.
    ...
    // Normal mode
    if ( digitAnimation == "Sweep" ) {                                // Some digit animations require the timeMatrix not to be updated with a color
      useOld = true;
    } else {
      useOld = false;
    }
...
//    timeMatrix = getTimeMatrix("none", digitColor);
    ...
    if (activity == "dotsBeat") {
      ...
      EVERY_N_MILLIS(10) {
        if ((onOffBG & 1) == 0) {                                         // even or odd.. blink of not
          int rndNum = rand() % 5;
          if (rndNum < 2) {                                               // by introducing a random change.. the blinks will become random
  ...
  */
  ...
  if ( !nightMode ) {                                                   // in nightmode no digit animation will happen.
    if (digitActivity == "Plasma") {
      ...
      EVERY_N_MILLIS(10) {
        if ((onOffFG & 1) == 0) {                                         // even or odd.. blink of not
          int rndNum = rand() % 12;
          if (rndNum < 2) {                                               // by introducing a random change.. the blinks will become random
