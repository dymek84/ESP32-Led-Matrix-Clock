...
std::map<int, CRGB> beatWave(std::map<int, CRGB> Matrix) {
  ...
  for (auto const& t : Matrix) {              // make from the map and array int[lednumber]=realposition
...
//plasma()
std::map<int, CRGB> plasma(std::map<int, CRGB> fg, boolean isTime = false) {                                                 // This is the heart of this program. Sure is short. . . and fast.
  int thisPhase = beatsin8(6, -64, 64);                         // Setting phase change for a couple of waves.
  int thatPhase = beatsin8(7, -64, 64);

  int k = 0;
  for (auto const& t : fg) {                            // make from the map and array int[lednumber]=realposition
  ...
  if ((Seconde & 1) == 0 && isTime) {
    fg[xyToLedNumber(15, 1)] = leds[xyToLedNumber(15, 0)];
    fg[xyToLedNumber(16, 1)] = leds[xyToLedNumber(16, 0)];
    fg[xyToLedNumber(15, 2)] = leds[xyToLedNumber(15, 3)];
    fg[xyToLedNumber(16, 2)] = leds[xyToLedNumber(16, 3)];

    fg[xyToLedNumber(15, 5)] = leds[xyToLedNumber(15, 4)];
    fg[xyToLedNumber(15, 6)] = leds[xyToLedNumber(16, 4)];
    fg[xyToLedNumber(16, 5)] = leds[xyToLedNumber(15, 7)];
    fg[xyToLedNumber(16, 6)] = leds[xyToLedNumber(16, 7)];
